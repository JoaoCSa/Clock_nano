...
//-----------Graveyard-------------------
  ...
  Serial.print(" ");*/
  //display.showNumberDecEx(0, 0b01000000, false, false);
    /*
  if (cycle_state==0 && digitalRead(ButConf)==LOW){
    cycle=1;
    cycle_state=1;
  } else if (cycle_state==1 && digitalRead(ButConf)==HIGH){
    cycle_state=2; 
  } else if (cycle_state==2 && digitalRead(ButConf)==LOW){
    cycle=0;
    cycle_state=3; 
  } else if (cycle_state==3 && digitalRead(ButConf)==HIGH){
    cycle_state=0; 
  } 
  //Neste momento o teste é para os segundos apenas. Quando cycle=1 e no FE do botão, os segundos devem alternar entre "__" e "ss"
  */
  ...
  /*
    ...
    Serial.print(" ");
    Serial.print(blinki_state);
    Serial.print(") ");
    */

       
    //display.showNumberDecEx(minute/10, 0b01000000, false, 1, 2);
    //display.showNumberDecEx(hour%10, 0b01000000, false, 1, 1);
    //display.showNumberDecEx(hour/10, 0b01000000, false, 1, 0);  
    /*
    display.showNumberDec(minute%10, false, 1, 3);
    display.showNumberDec(minute/10, false, 1, 2);
    display.showNumberDec(hour%10, false, 1, 1);
